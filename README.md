...
EBM学習付きデモでは以下を比較します：
1. 未学習EBM: ランダムなパラメータ＋弱いバイアス（+26.6%改善）
2. 単一パターン学習: ターゲットパターンのみで学習したEBM（**+39.1%改善、90.6%精度達成！**）
3. 複数パターン学習: 複数パターンで学習したEBM（-3.1pp、汎化の難しさを示す）

このデモは、**Contrastive Divergence**アルゴリズムによるEBM学習が
デノイジング性能を**劇的に**向上させることを実証します。
特に、ターゲットパターンが既知の場合、そのパターンで学習すると**90%超の精度**を達成できます。
